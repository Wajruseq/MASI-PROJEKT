...
class CanvasWidget(Widget):
    def add_text(self, text, x, y, tags=None, is_replacement=False):
        with self.canvas:
            # Ustaw kolor dla tekstu
            if is_replacement:
                Color(1, 0, 0, 1)  # Czerwony dla term_P_A/term_P_B
            else:
                Color(0, 0, 0, 1)  # Czarny dla term_A/term_B
...
class MainScreen(BoxLayout):
    ...
    def __init__(self, **kwargs):
        ...
        self.delete_popup = None  # Store reference to Delete popup
    ...
    def savef(self):
        ...
        operator_p = ';'  # Domyślna wartość dla OP_P
    ...
    def draw_sequencing(self, term_a, term_b, operator, term_a_replaced=False, term_b_replaced=False):
        ...
        text_width = max(len(result) * 10, 100)  # Minimalna szerokość tekstu
        margin = 20
        x1 = 80
        ...
        with self.ids.canvas_widget.canvas:
            Color(0.68, 0.85, 0.9, 1)  # Kolor łuku
            Line(ellipse=(x1, y1, x2, y2, 0, 180), width=2)
        # Rysuj podstawową sekwencję (czarny tekst)
            self.ids.canvas_widget.add_text(result, x1 + (x2 - x1) / 2 - text_width / 2, y2 - 35)
            # Rysuj zamienione unitermy na czerwono, jeśli dotyczy
            if term_a_replaced:
