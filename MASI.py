...
class MainScreen(BoxLayout):
    ...
    def __init__(self, **kwargs):
        ...
        self.load_popup = None  # Store reference to Load popup
    ...
    def draw_sequencing(self, term_a, term_b, operator, term_a_replaced=False, term_b_replaced=False):
        ...
        text_width = max(len(result) * 10, 100)  # Ensure minimum width
        margin = 20
        x1 = 80
        ...
        with self.ids.canvas_widget.canvas:
            ...
            # Center the entire sequence
            self.ids.canvas_widget.add_text(result, x1 + (x2 - x1) / 2 - text_width / 2, y2 - 35)
            # Overlay replaced uniterms in red if applicable
            if term_a_replaced:
                term_a_width = len(term_a) * 10
